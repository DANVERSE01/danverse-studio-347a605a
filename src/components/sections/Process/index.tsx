...
const phases = [
  {
    ...
    duration: '1–2 weeks', accent: 'pearl',
  },
  {
    ...
    duration: '2–3 weeks', accent: 'pearl',
  },
  {
    ...
    duration: '4–8 weeks', accent: 'pearl',
  },
  {
    ...
    >
      ...
      <div className="absolute top-1/2 right-8 md:right-16 -translate-y-1/2 pointer-events-none select-none">
        <span
          ...
          style={{
            ...
            WebkitTextStroke: '2px hsl(var(--rose-gold) / 0.06)',
      ...
      <motion.div ref={inViewRef} variants={staggerContainer} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="mb-24">
        <motion.div variants={fadeUp} className="flex items-center gap-4 mb-6">
          ...
          <div className="w-16 h-px" style={{ background: 'hsl(var(--rose-gold) / 0.2)' }} />
        </motion.div>
        <motion.h2 variants={fadeUp} className="flex items-baseline gap-3 md:gap-5 flex-wrap">
          <span className="font-display italic font-light tracking-[0.01em]" style={{ fontSize: 'var(--text-section)', color: 'hsl(var(--pearl))' }}>
            How we
          </span>
          <span className="font-heading font-extrabold uppercase tracking-[-0.04em]" style={{
            fontSize: 'clamp(3rem, 8vw, 8rem)',
            WebkitTextStroke: '2px hsl(var(--rose-gold) / 0.5)',
            WebkitTextFillColor: 'transparent',
          }}>
            THINK
          </span>
  ...
  return (
    ...
    >
      <div className="flex md:flex-col items-baseline md:items-start gap-3">
        <span className="font-display italic text-5xl md:text-6xl transition-colors duration-500" style={{ color: 'hsl(var(--rose-gold) / 0.2)' }}>
          {phase.num}
        </span>
        <span className="font-mono-brand text-[9px] uppercase tracking-[0.25em]" style={{ color: 'hsl(var(--rose-gold) / 0.6)' }}>
          {phase.label}
        </span>
      </div>

      <div>
        <h3 className="font-display italic text-2xl md:text-3xl leading-[1.2] transition-colors duration-500 group-hover:text-foreground" style={{ color: 'hsl(var(--pearl) / 0.8)' }}>
          {phase.title}
        </h3>
        <p className="text-[12px] leading-[1.9] mt-3 max-w-sm" style={{ color: 'hsl(var(--pearl) / 0.4)' }}>
          {phase.description}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap gap-x-5 gap-y-2">
          {phase.deliverables.map((d) => (
            <span key={d} className="flex items-center gap-2 text-[11px]" style={{ color: 'hsl(var(--pearl) / 0.5)' }}>
              <span className="w-1 h-1 rounded-full" style={{ background: 'hsl(var(--rose-gold) / 0.3)' }} />
              {d}
            </span>
          ))}
        </div>
        <span className="font-mono-brand text-[9px] tracking-[0.2em] uppercase" style={{ color: 'hsl(var(--pearl) / 0.25)' }}>
          {phase.duration}
        </span>
      </div>
    </motion.div>
