...
export default function SectionDivider({ variant = 'morph-wave', accent = 'coral', className = '' }: SectionDividerProps) {
  ...
  // Mobile: simple static line divider (no scroll-driven SVG/rotation)
  if (isMobile) {
    return (
      <div className={`relative w-full flex items-center justify-center ${className}`} style={{ height: 60 }}>
        <div className="w-3/4 h-px" style={{ background: `linear-gradient(90deg, transparent, hsl(var(--${accent}) / 0.15), transparent)` }} />
        <div className="absolute w-1 h-1 rounded-full" style={{ background: `hsl(var(--${accent}) / 0.3)` }} />
      </div>
    );
  }
...
function MorphWave({ progress, accent }: { progress: any; accent: string }) {
  ...
  return (
    <svg viewBox="0 0 1000 120" className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
      <motion.path d={path} fill="none" stroke={`hsl(var(--${accent}) / 0.15)`} strokeWidth="1" />
    </svg>
  );
}
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      <motion.div
        ...
        style={{ width: '80%', scaleX, opacity, background: `linear-gradient(90deg, transparent, hsl(var(--${accent}) / 0.3), transparent)` }}
      />
      <motion.div
...
function Diamond({ progress, accent }: { progress: any; accent: string }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      <motion.div className="w-8 h-8 border" style={{ borderColor: `hsl(var(--${accent}) / 0.2)`, rotate, scale, opacity }} />
    </div>
  );
}
...
function OrbitingDots({ progress, accent }: { progress: any; accent: string }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      <motion.div className="relative w-16 h-16" style={{ rotate: rotation }}>
        {[0, 90, 180, 270].map((angle) => (
          <div key={angle} className="absolute w-1 h-1 rounded-full"
            style={{ background: `hsl(var(--${accent}) / 0.4)`, top: '50%', left: '50%', transform: `rotate(${angle}deg) translateX(32px) translate(-50%, -50%)` }}
          />
        ))}
      </motion.div>
    </div>
  ...
  return (
    <div className="absolute inset-0 flex items-center">
      <div className="w-full h-px relative overflow-hidden" style={{ background: `hsl(var(--${accent}) / 0.04)` }}>
        <motion.div className="absolute inset-y-0 w-1/3" style={{ x, background: `linear-gradient(90deg, transparent, hsl(var(--${accent}) / 0.3), transparent)` }} />
      </div>
    </div>
  );
}
